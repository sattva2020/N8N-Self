...
server.on('error', (err) => {
  if (err && err.code === 'EACCES') {
    ...
    console.error(
      `Permission denied binding to port ${port} (host tried: ${host}${triedFallback ? ', fallback attempted' : ''}).`
    );
    console.error(
      'If this persists locally, try: run PowerShell as Administrator, or start the server with --host 0.0.0.0'
    );
    process.exit(1);
  } else {
